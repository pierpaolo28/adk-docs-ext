...
This project supports autonomous development by AI coding agents. Follow this workflow to build, evaluate, test, and deploy the agent.
...
1. Create evaluation cases in `tests/eval/evalsets/`
2. Run evaluations: `make eval`
3. Discuss results with the user
4. Add more test cases for edge cases and new scenarios
5. Adjust prompts, tools, or agent logic based on results
...
**google_search doesn't appear in tool_uses:**
The `google_search` built-in tool uses `GoogleSearchRetrieval` internally, which doesn't register as a standard function call in the trajectory. If your evalset expects `google_search` in `expected_tool_calls`, it will fail even if search results appear in the response. Instead, check for grounded search content in the response.
...
# - On setup_cicd: Terraform runs for staging and prod
...
Note: `setup_cicd` automatically initializes git if needed.

**Steps:**
1. Use `setup_cicd` tool with staging and production GCP project IDs
2. Creates infrastructure in BOTH staging and production projects
3. Sets up Cloud Build or GitHub Actions triggers
...
| Runner | Pros | Cons |
|--------|------|------|
| **github_actions** (Default) | No interactive auth, uses WIF, fully automated | Requires GITHUB_PAT environment variable |
| **google_cloud_build** | Native GCP integration | Requires interactive browser authorization |

### After CI/CD Setup: Activating the Pipeline

**IMPORTANT**: `setup_cicd` creates infrastructure but doesn't deploy the agent automatically.
...
OPTIONS (endpoint = 'https://amadeus-flight-assistant.run.app/invoke');
...
gcloud pubsub subscriptions create my-subscription \
    --topic=my-topic \
    --push-endpoint=https://amadeus-flight-assistant.run.app/invoke
```
...
gcloud eventarc triggers create storage-trigger \
    --destination-run-service=amadeus-flight-assistant \
